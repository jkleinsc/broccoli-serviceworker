...
var BroccoliServiceWorker = function BroccoliServiceWorker(inTree, options) {
  ...
  this.customCache = options.customCache || false;
...
BroccoliServiceWorker.prototype.write = function(readTree, destDir) {
  ...
  var customCache = this.customCache;
  ...
  return readTree(serviceWorkerTree).then(function (srcDir) {
    ...
    lines.push("var CACHE_PREFIXES = [");
    lines.push("  'prefetch-cache-v',");
    if (dynamicCache.length) {
      lines.push("  'dynamic-cache-v',");
    }
    lines.push("];");
    lines.push("var CURRENT_CACHES = {");
    lines.push("  prefetch: 'prefetch-cache-v' + CACHE_VERSION,");
    if (dynamicCache.length) {
      lines.push("  dynamic: 'dynamic-cache-v' + CACHE_VERSION,");
    }
    lines.push("};");
    ...
    lines.push("          return CACHE_PREFIXES.findIndex(function(cachePrefix) {");
    lines.push("            return cacheName.indexOf(cachePrefix) === 0;");
    lines.push("          }) !== -1;");
    ...
    addDebugLine("'Found response in cache for ' + event.request.url + ':', response", debug, lines);
    if (fallback.length) {
    ...
    lines.push("      }");
    addDebugLine("'No response found in cache for ' + event.request.url + '. About to fetch from network:'+event.request", debug, lines);
    ...
    if (fallback.length) {
      ...
      lines.push("        console.error('Fetching failed for ' + event.request.url + ':', error);");
